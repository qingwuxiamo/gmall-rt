...
public class CanalClient {
    ...
    public static void main(String[] args) throws InvalidProtocolBufferException {
        ...
        //1.获取canal连接器
        CanalConnector canalConnector = CanalConnectors.newSingleConnector(new InetSocketAddress("hadoop105", 11111), "example", "", "");
        //死循环，一直监控数据库，否则当main方法执行完毕，连接就断掉了。
        while (true) {
            //2.通过连接器获得canal连接
            ...
            if (entries.size() > 0) {
                ...
                for (CanalEntry.Entry entry : entries) {
                    ...
                    //10.判断entry类型，如果是ROWDATA则处理（ROWDATA才是数据）
